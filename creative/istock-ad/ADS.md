...
`Ad.tsx` construye cualquier spec; `Root.tsx` valida (`src/ads/validate.ts`) y registra una `Composition` por spec: un spec inválido no se lista, no se renderiza y no se buildea.

**Promesa fija:** el anuncio dice que iStock te da **tu propia landing** y el único host que se ve es `istock.maat.work` (chip, mensaje de WhatsApp, cierre). Ningún host de tenant, ni el seed, ni `localhost`: lo rechazan el validador, el lint y el guard de captura.
...
|---|---|---|---|---|
| `IstockReelV10` | `reel-v10` | todas las noches las mismas preguntas → cargá una vez, tu propia landing, ficha, WhatsApp | 18.0 s | night |
...
## Reglas que valida `validate.ts`

- id `Istock<Nombre>` y slug derivado del id; ids únicos.
- 8–24 s en total (las camas duran 24 s); cada escena ≥ 45 frames.
- Abre con un hook, cierra con `close` que lleva `Probalo 14 días gratis.`, y muestra el teléfono al menos una vez.
- Presupuesto de caracteres por texto (título 52, línea de titular 18, sub 44, línea de cierre 40, mensaje 34) para que nada se salga de su caja.
- Highlight dentro de la pantalla del teléfono y sin scroll; `at` dentro de la escena.
- Copy sin `altovalle`, `demo.maat.work`, `iStock Demo`, `localhost`, `imei`, `costo`, `margen`; el único `*.maat.work` permitido es `istock.maat.work`.
...
3. `pnpm typecheck && pnpm lint && pnpm specs`; después `pnpm qa <id>`: un still por escena (y
   uno por highlight) y la tira `out/qa/<slug>-scenes.png`, mirada con los ojos.
