...
> **Y queda una contradicción abierta que `docs-keeper` registra sin arbitrar:** el LEAD despachó el
> cierre diciendo que con eso `packages/ai` queda **sin** entrada en `doing`, y **`T19` sigue marcada
> `doing` sobre el mismo directorio** (fila de FASE 4 bis, `ai-agent`, bloqueada por **B4**). O el
> `doing` de `T19` es un rastro viejo —su celda lo justificaba con *«`ai-agent` tiene el paquete
> abierto mientras se escribe esto»*, y el árbol al 2026-08-28 está **limpio**, `git status` sin
> cambios—, o el conteo de `doing` sobre `packages/ai` no es cero. **Lo decide el LEAD, no este
> documento**, que es la misma razón por la que la colisión original tampoco la arbitró.
