...
**El gate ya corre entero y cierra verde. Las cinco filas siguen en `todo` igual, y el motivo
cambió:** ya no es que el gate no pudiera correr, es **quién lo corrió**. `bash scripts/accept-fase3.sh`
→ **`FASE 3: ACEPTADA` · exit 0 · 54 PASS · 0 FAIL**, `next build` incluido, el 2026-08-28 sobre el
árbol de trabajo y en la versión modificada que registra la fila **T29**. La corrió `docs-keeper`,
**no el LEAD**, así que por `CLAUDE.md` §0.2 no promueve nada: lo que fija el estado de K1–K5 es la
re-ejecución del LEAD, y esa no existe todavía.
...
> listó las rutas (`○ /`, `ƒ /_media/[...key]`, `ƒ Proxy (Middleware)`). Lo único pendiente es la
> corrida del LEAD.
...
> abiertas **S2.5** (el guard de IMEI de `packages/media` rechaza el 1,88% de las fotos, **para
> siempre**, y ataca el *done cobrable*), **T17** (la reserva configurable por tenant que `PRODUCT.md`
> promete y no existe), **T18** (el tercer call site que ADR-019 no alcanzó — **cerrada el 2026-08-28**) y **T19**
...
| T12 | editar el TC y los puntos de retiro después del alta **no existe** | todo | `app-agent` | — | el dueño cambia el TC y edita/agrega un punto de retiro desde el panel **sin recrear el negocio**, y la mutación arrastra la invalidación de la vidriera. **Requisito de aceptación que nace ANTES que la slice, agregado el 2026-08-28 al cerrar S6.2:** la mutación **tiene que llamar `invalidateStorefront()`**, no `updateTag(storefrontTag(slug))`. Desde S6.2 `tenant-config:{slug}` es el **único** tag de alcance tenant que le queda a la ficha, y `invalidateStorefront()` tiene **un solo caller en todo el repo** (`create-tenant.ts:378`): emitir `storefront:{slug}` a mano deja **cada ficha del tenant con el TC viejo hasta un año** por `cacheLife('max')`, sin error y sin log. Ver §T12 | `apps/web/app/(app)/app/(panel)/ajustes/` · el contrato de tags: `apps/web/app/(app)/_lib/tenants/storefront-cache.ts` |
| T17 | la reserva **configurable por tenant** está prometida y no existe | todo | `app-agent` | — | `PRODUCT.md` Q3 promete *"duración default de la reserva, configurable"*; hoy el rango y el default son **constantes del dominio** (`RESERVATION_MIN/MAX/DEFAULT_MINUTES`) más el `CHECK` `reservations_minutes_range`, iguales para todos los tenants. Cerrada cuando el dueño fija la duración default de **su** negocio desde el panel, dentro de \[30, 120\], y fuera de rango se sigue **rechazando y no clampeando**. **Sin fecha** — no bloquea ninguna slice de FASE 4 | `packages/domain/src/reservation.ts` · `apps/web/app/(app)/app/(panel)/ajustes/` |
...
| P5 | **ninguna** policy de `sales` **ni de `tradein_leads`** mira `membership_role`: a nivel base de datos un `seller` lee `cost_usd`, `margin_usd` y ahora también `offer_usd` e `internal_notes` de su tenant | todo · **prioridad ALTA** · **creció con S8 y quedó MEDIDO otra vez, en otra tabla** | `db-agent` | — | **Medido por el LEAD el 2026-08-28 con `psql` contra `istock_dev`**, y verificado por `docs-keeper` contra el SQL: las cuatro policies (`sales_tenant_select`, `_insert`, `_update`, `_delete`, `packages/db/drizzle/0000_sparkling_vector.sql:445-448`) comparan **sólo** `tenant_id` contra el claim de `app_metadata`; `membership_role` aparece en el repo como enum y **cero veces** en un predicado de policy. Eso contradice `CLAUDE.md` §0.9 (*"Seller no ve costo ni margen. **Nunca**"*) **en la capa de la base**. **No es alcanzable hoy** porque no hay cliente Supabase en el browser (**B2**): el único que abre esas conexiones es el servidor, y el camino de venta no devuelve costo ni margen a nadie —ni al dueño—, afirmado en `D6`/V6 de `accept-s7.sh` sobre el payload real. **Por qué es alta igual, y es el único motivo que hace falta:** §0.9 es **el único invariante de producto que hoy cuelga de una sola capa**. Todos los demás tienen dos —la query filtra por tenant *además* de RLS, el DTO es allowlist *además* de la policy—; éste tiene el servidor y nada abajo. El día que aparezca un cliente de browser —panel realtime, un `select` desde el cliente autenticado— deja de ser una asimetría y pasa a ser una fuga, sin que nadie tenga que equivocarse. **S11 no lo cubre:** S11 es el modelo de permisos del **panel**, o sea más servidor. Esta fila es la capa de abajo. **S8 la ensanchó a una segunda tabla, y esto es MEDIDO por el LEAD contra Postgres real, no reportado por el agente que escribió el código:** un `seller` autenticado **sí puede** leer `offer_usd` e `internal_notes` de `tradein_leads` — `membership_role` aparece **cero veces** en las policies de esa tabla, igual que en `sales`. `offer_usd` **es** el costo: es lo que el dueño paga por el equipo del visitante. O sea que la regla 9 de `CLAUDE.md` (*"seller no ve costo ni margen"*) **sobre esta tabla la sostiene hoy el servidor, no la base**, exactamente como sobre `sales`. **Lo que SÍ está atado, y lo verificó el `adversary-reviewer`, se escribe con el mismo detalle porque es lo único que hay:** `listTradeinLeads()` devuelve una **unión discriminada por `canSeeOffer`**, y en la rama del `seller` la clave `offerUsdCents` **no existe en el objeto** —no es `null`: está ausente, y un `null` serializado seguiría diciendo que el campo existe— y el SQL de esa rama **no nombra** `offer_usd` ni `internal_notes` en ningún lado. **No es un `if` de render: leer el campo en esa rama no compila.** Y el adversary censó que **no existe un tercer camino de lectura** — un `grep -rn` de los dos nombres (`tradeinLeads` y `tradein_leads`) sobre `apps/web` da exactamente **dos** consumidores. **Que P5 siga abierta se declara, no se tapa:** V5 de `accept-s8.sh` mide `costo_en_el_payload_del_seller=0` **sobre el objeto** que devuelve la lectura y no sobre su tipo, precisamente porque el tipo lo cumple el compilador y el objeto lo cumple el código que corre — es la única cosa que sostiene el invariante mientras la policy no exista. | `packages/db/drizzle/0000_sparkling_vector.sql:445-448` · `packages/db/src/schema/commerce.ts` (`tenantPolicies`) · `packages/db/src/schema/tradein.ts` · `apps/web/app/(app)/_lib/tradein/queries.ts` (la unión discriminada) · `scripts/accept-s8.sh` V5 · blocker **B2** |
...
| T39 | `accepts_trade_in` no tiene UI de edición: un dueño que quiere **dejar** de recibir canjes no puede | todo | `app-agent` (`apps/web/app/(app)/**` por §4) | — | **Es producto, no cosmética**, y la asimetría es la fila: el flag se decide **una vez**, en el signup, y después es de sólo lectura para siempre. Censado contra el árbol: el **único** writer es `create-tenant.ts` (el alta del negocio), y `app/(app)/app/(panel)/ajustes/page.tsx:76` lo muestra como `DataRow` read-only —*"Tomás equipos en canje: Sí/No"*— detrás del mismo `<NotReadyYet>` que tapa el TC y los puntos de retiro (`:97`). **Por qué importa más que un campo cualquiera:** el flag es lo que decide si la vidriera publica el formulario público de canje, o sea que un dueño que se llenó de leads o que dejó de tomar canjes **no tiene manera de cerrar la puerta** sin pedirle a alguien que le toque la base. Hermana de **T12** (el TC tampoco se puede editar después del alta) y del mismo `<NotReadyYet>` | `apps/web/app/(app)/app/(panel)/ajustes/page.tsx:76` y `:97` · `apps/web/app/(app)/_lib/tenants/create-tenant.ts` (el único writer) · `apps/web/app/(storefront)/s/[slug]/canje/page.tsx` (el consumidor del flag) |
| T40 | `readBody()` **bufferea el cuerpo entero** antes de aplicar el techo de 6144 caracteres | todo · **severidad baja** · lo que la hace fila es que **hoy no está declarado en ningún lado** | `storefront-agent` (`apps/web/app/(storefront)/**` por §4) | — | **Levantado por `adversary-reviewer` en S8.** `readBody()` hace `await request.text()` y **recién después** compara contra `MAX_TRADEIN_BODY_CHARS` (`apps/web/app/(storefront)/s/[slug]/api/tradein/route.ts:98-101`; la constante es `6144`, `_lib/tradein-form.ts:89`). O sea que el techo se aplica sobre un string que ya está en memoria. **Acotado por el techo de 4.5 MB de la plataforma, y NO es una regresión: es el mismo patrón que `/api/track` de S4**, así que la fila no acusa a la slice, acusa a la ausencia de una decisión escrita. **Las dos salidas, y la elección es de quien la escriba:** cortar por `content-length` **antes** del `await` —rechazando también el `content-length` **ausente**, porque si no el bypass es no mandarlo— o **declarar el patrón aceptado** con el mismo argumento que S4 y dejarlo dicho una sola vez para las dos rutas. Lo que no puede seguir es lo de hoy: dos endpoints con el mismo patrón y cero líneas que digan si es a propósito | `apps/web/app/(storefront)/s/[slug]/api/tradein/route.ts:98-101` · `apps/web/app/(storefront)/_lib/tradein-form.ts:89` y `:191` · `apps/web/app/(storefront)/s/[slug]/api/track/route.ts` (el precedente de S4) · `docs/research/vercel-request-body-limit.md` |
...
| T47 | **`apps/web/proxy.ts:363-365` explica una fragilidad del parser de matchers que ya no existe** | todo · **severidad baja, es un comentario** | `storefront-agent` (`proxy.ts` por §4) | — | El comentario le pide al que edite el array `matcher` que no escriba corchetes adentro de los comentarios, *"porque el guard lo lee del FUENTE con un regex no-goloso que corta en el primer corchete de cierre"*. **Ese regex ya no es el que corre.** `tests/proxy-matcher-no-deja-la-vidriera-sin-vigilar.test.ts` documenta en su propio docblock que *"la versión anterior era `/matcher\s*:\s*\[([\s\S]*?)\]/u`"* y hoy el lector es un escáner que entiende literales de string y falla explícito con *"el `matcher` tiene un literal de string sin cerrar"*. **Por qué una fila y no un `sed`:** un comentario que pide una precaución por un motivo falso es peor que ninguno — la próxima persona que lo lea va a evitar algo que no hace falta y, sobre todo, va a **confiar** en que el resto del docblock está al día. Y es del `storefront-agent`, que no lo puede arreglar `docs-keeper` ni el LEAD sin pisarle la columna. **Ojo:** la fragilidad **como clase** sigue abierta y es `T7`; lo que caducó es la explicación del mecanismo | `apps/web/proxy.ts:363-365` · `tests/proxy-matcher-no-deja-la-vidriera-sin-vigilar.test.ts` (§"lectura del matcher") · fila `T7` |
| T48 | **el header de `packages/db/drizzle/0009_...sql` se autotitula `S9`, y `0009` es la fila `S8.1`** | todo · **severidad baja** · es una línea de comentario | `db-agent` (`packages/db/**` por §4 — **`docs-keeper` no lo arregla: es su columna**) | — | La primera línea del cuerpo del header dice `-- 0009 · S9 · Dos afirmaciones del canje…`. **`S9` es otra slice**: *"copy list para estados de IG/WA"*, de `app-agent`, todavía en `todo` (FASE 4). Lo que la `0009` implementa es `S8.1`. **Es chica y es exactamente el tipo de cosa que confunde después:** el que busque la migración de S9 la va a encontrar y va a leer un canje, y el que audite S8.1 va a buscar un archivo que dice S8.1 y no lo va a encontrar. El número de archivo (`0009`) y el número de slice (`S9`) son series distintas que acá se pisaron por parecerse — primo de `T44` un nivel más abajo | `packages/db/drizzle/0009_tradein_accepts_and_acquisition_channel.sql:2` |
| T49 | **el soft cap del chat es una cuota compartida entre el comprador real y quien la queme** | todo · **abierta** · no bloquea nada hoy: `chatbot-rl` sigue en `planned` y `/api/chat` no existe. **Aterriza con FASE 5** | `ai-agent` (`packages/ai/**` por §4) para la **forma del contador** · la **política** es del **LEAD** | — | **`SOFT_CAP_MESSAGES_PER_TENANT_PER_DAY = 40` es por tenant, no por sesión ni por IP** (`packages/ai/src/entitlement.ts`, fijado en `entitlement.test.ts:112`). Con `chatbot-rl` tal como está — 20 req / 600 s por IP, `config/firewall-rules.json` — **una sola IP agota los 40 mensajes del día de un tenant en veinte minutos** y le deja el chatbot mudo al reseller por el resto de la jornada: gratis, sin violar ninguna regla, y sin que ninguna alarma suene. **No es un agujero de costo:** por encima del cap el chat devuelve handoff con `provider: 'none'`, o sea cuesta cero. Es un **agujero de disponibilidad de la feature que el plan Negocio cobra**. **El arreglo no es apretar el WAF, y eso está evaluado y descartado por el LEAD:** bajar a 10/600 s corta a un comprador real en la pregunta 11, y un comprador informado que abre WhatsApp **es** el producto — se prefiere la falla barata sobre la cara, igual que en la excepción del cron. **El arreglo es que el contador distinga sesiones, y esa decisión todavía no está tomada.** Es fila de board, no número de WAF | `packages/ai/src/entitlement.ts` (`SOFT_CAP_MESSAGES_PER_TENANT_PER_DAY`) · `packages/ai/src/chat.ts:249` · `config/firewall-rules.json` (`chatbot-rl`, su `why` lo explica entero) · `docs/COST.md` §2.6 y `C1` |
| T50 | **el techo de llamadas facturadas por turno es 4, y no hay constante ni test que lo diga** | **doing** · encargo despachado por el LEAD el 2026-08-28 · alias `C11` de `COST.md` · **entrada única de `doing` con `S8.2`**, resuelta por el LEAD el 2026-08-28: esta fila llegó a registrar dos `doing` sobre `packages/ai` —contra la regla de §Estados— y **se arregló haciendo cierta la regla, no aflojándola**: `S8.2` no tenía a nadie encima y se sumó a este mismo encargo. Un writer, un encargo, dos ítems. Queda anotado porque la próxima pasada va a encontrar el rastro de la colisión y va a querer saber si hubo excepción: **no la hubo** | `ai-agent` (`packages/ai/**` por §4) | — | **Origen: `cost-auditor`, `docs/COST.md` §2.8.3. Medido, no leído.** `generateWithFallback` (`packages/ai/src/chat.ts:204`) cuenta como *atendida* la llamada del primario que devuelve `200` con texto vacío —y hace bien: el proveedor procesó el prompt y lo factura— y **recién después** cae al fallback. Como `answerChat` la invoca **dos veces** —ronda inicial (`:301`) y ronda post-tool (`:378`), con `MAX_TOOL_ROUNDS = 1` en `:50`—, el techo estructural del turno son **4 llamadas facturadas, no 2**. `cost-auditor` lo midió con un primario stub que contesta vacío: `billed: { calls: 4, tokensIn: 4274 }` sobre la ficha del plan Negocio. **Precio:** USD 0,000672/mensaje → **USD 0,8064/mes** al soft cap, o sea **1,24× bajo el USD 1,00 de chat en vez de 2,2×** — el primer techo de `COST.md` que queda a menos de 1,5× de su objetivo. **Lo que esta fila afirma es el invariante, no la implementación: el techo de llamadas facturadas por turno tiene que quedar clavado por un test.** Hoy no lo dice nada: `chat.test.ts` fija `billed.calls` en 1 y en 2 para escenarios puntuales (`:317`, `:336`, `:354`) y **ninguna aserción acota el peor caso**. Sin ella, **un tercer proveedor de respaldo lleva el turno a 6 llamadas —USD 0,000960/msg, +43%— sin tocar una constante de `budget.ts` y sin que nada se ponga rojo**, y agregar un fallback suena a robustez, no a cambio de costo: es exactamente la clase de cambio que nadie va a mandar a costear. **Qué palanca se usa para bajar el techo NO lo decide esta fila:** `C11` propone no reintentar el primario en la segunda ronda si ya falló en la primera (3 llamadas, USD 0,5760/mes, −29% del techo absoluto, sin tocar dieta ni soft cap ni calidad); es de `ai-agent` proponerla y del LEAD ratificarla. **Todas las líneas de esta fila están medidas contra HEAD `6aea02b`, y el árbol de trabajo ya no dice eso: al 2026-08-28 `ai-agent` tiene el arreglo EN VUELO y SIN COMMITEAR** —`MAX_BILLED_CALLS_PER_TURN` exportado por `index.ts`, un `skipPrimary` en `generateWithFallback` y aserciones nuevas en `chat.test.ts`—, así que las citas de línea se mueven y **la fila la cierra la corrida del LEAD, no la lectura del árbol** | `packages/ai/src/chat.ts:50` (`MAX_TOOL_ROUNDS`) · `:204-241` (`generateWithFallback`) · `:290-294` (`addBilled`) · `:301` y `:378` (las dos rondas) · `packages/ai/src/chat.test.ts:310-355` · `packages/ai/src/budget.ts` · `docs/COST.md` §2.8.3 y `C11` |
| T51 | **`billed` se calcula y no lo lee nadie: nadie lo emite y nadie alarma cuando el turno pagó de más** | todo · **`C10` la describía en dos mitades; censadas, queda UNA** — la de `ai-agent` ya está hecha (celda de al lado) · alias `C10` de `COST.md` | `app-agent` (emitirlo y alarmar, `apps/web/app/api/**` y `(app)/**` por §4) · `ai-agent` (`packages/ai/**`) **sólo si la pregunta abierta de la celda de al lado se contesta que sí** — su mitad, tal como venía descrita en `C10`, ya está hecha | la mitad de `app-agent` espera **FASE 5**: `/api/chat` **no existe** (censado el 2026-08-28: `apps/web/app/api` tiene `cron`, `health` y `tenants`, y `answerChat` no tiene un solo llamador fuera de `packages/ai/src`). **Esa espera es toda la fila:** lo que `C10` le pedía a `ai-agent` ya está en el árbol | **Origen: `cost-auditor`, `docs/COST.md` §2.8, alias `C10`.** `ChatAnswer.billed` existe desde `C8` —cuenta las llamadas que un proveedor **atendió** en el turno, suma y no máximo— y **hoy no lo lee nadie**. **La fila es una sola, con la dependencia escrita adentro en vez de partida en dos, porque el valor no aparece hasta que alguien emite:** un campo expuesto que nadie lee no mide nada, y emitirlo es imposible antes de que exista la ruta. (a) **`ai-agent`**: el encargo la describía como *«exponer el campo, que hoy se descarta en el borde»*, y **censado contra HEAD `6aea02b` esa mitad ya está hecha** — `packages/ai/src/index.ts` exporta `type BilledUsage` y `type ChatAnswer`, que es la que lleva `billed`. **Y nada se descarta en ningún borde, porque todavía no hay borde:** `answerChat` no tiene un solo llamador fuera del paquete. **Lo que queda del lado de `ai-agent` es una pregunta abierta y `docs-keeper` no la cierra:** si el emisor necesita algo más que el campo —el modelo y el proveedor de **cada** llamada, para poder atribuir un `calls > 2` a quién contestó vacío— eso sí sería trabajo suyo, y no está decidido ni medido. (b) **`app-agent`**: se emite al log estructurado (`logEvent`, `apps/web/app/(app)/_lib/log.ts`) con **alarma en `calls > 2`** — se despacha **cuando la ruta se escriba, no antes; esta fila no inventa `/api/chat` ni su forma**. **Por qué el umbral es `> 2` y no `> 4`:** 2 es el techo del camino feliz con tool (una llamada por ronda), y 3 o 4 sólo aparecen cuando el primario contesta vacío — que es el modo que **duplica la factura en silencio**, sin excepción en Sentry y sin que la respuesta se vea peor. **Y el síntoma es correlacionado, no individual:** el propio docblock de `chat.ts:198-202` llama a la respuesta vacía *«el modo de falla más común de un modelo barato bajo carga»*, y bajo carga le pasa a muchos turnos a la vez. Es el único indicador que distingue el techo real del techo que creíamos tener — el de `T50`. **Y una advertencia para el que la tome:** el trabajo en vuelo de `T50` agrega `MAX_BILLED_CALLS_PER_TURN` al mismo `index.ts`, sin commitear. **Eso tampoco cierra esta fila** — exportar la constante del techo no es emitir la medición del turno, y con las dos cosas en el árbol **sigue sin haber una sola línea de log** | `packages/ai/src/chat.ts:114-141` (`ChatAnswer.billed` y su docblock) · `packages/ai/src/index.ts` (el borde del paquete) · `apps/web/app/(app)/_lib/log.ts` (`logEvent`, el sink, con su denylist de PII) · `docs/COST.md` `C10` y §2.8.3 · fila `T50` (el techo que este log mediría) |
| T52 | **el techo de 1200 tokens de entrada: 7 de las 9 degradaciones del corpus son la ficha del plan Negocio** | **`blocked` · decisión humana** · alias `C9` de `COST.md`, re-medido · **no es un `B<n>`**: no bloquea ninguna slice, ver la nota de §"Blockers abiertos" | **humano** (el 1200 es de su goal: `≤1200 in / ≤180 out`) · `ai-agent` (`packages/ai/**`) ejecuta **si** se decide moverlo | **decisión humana pendiente.** El código funciona hoy: no espera a nadie, degrada. **Pero la decisión hoy se tomaría sobre un corpus sintético:** lo que la vuelve decidible sobre fichas de tenants reales es **`T53`** (`C6`), y la mitad de `T53` que da ese dato espera **FASE 5** | **Origen: `cost-auditor`, `docs/COST.md` §2.8.4 y §2.8.5. Esta fila NO propone subir el techo: registra que hay una decisión abierta, con su precio medido.** Barriendo techos sobre el corpus de `packages/ai`, **1374 es el mínimo exacto que deja los 162 prompts armados sin degradar** — a 1350 degrada uno y a 1300 cuatro (`COST.md` §2.8.4), y **el LEAD barrió el borde por su cuenta el 2026-08-28, aparte de `cost-auditor`: a 1373 ya degrada uno.** O sea que el 1374 es exacto, no redondeado, y **dos mediciones independientes coinciden en él** — que es la única razón por la que este número se puede usar para decidir. Hoy, con `MAX_INPUT_TOKENS = 1200` (`packages/ai/src/budget.ts:12`), **degradan 9 de 162, y 7 de esos 9 son la ficha del plan Negocio** —el tenant de USD 35—, que entra tirando **los 6 medios de pago y los 4 turnos de historial, o sea el historial entero**. **El que más paga es el único al que el chatbot se le olvida la conversación**, y le pasa justo en el turno en que el modelo pidió un dato para contestar mejor. **Precio de subirlo a 1374: entre USD 0,00047 y USD 0,0731/tenant/mes** contra una asignación de chat de USD 1,00 — entre 0,05% y 7,3%, **ruido en las dos ramas**. Y ahí está el punto: **si subir el techo cuesta ruido, el 1200 no se está pagando con plata, se está pagando con calidad, y una decisión que no se paga con plata no la puede arbitrar el auditor de costo.** `cost-auditor` declara explícitamente que **no tiene objeción a ninguna de las tres ramas** (subir el techo · achicar la ficha del Negocio · dejarlo como está), así que el que quiera sostener el 1200 no tiene que pelear contra ningún número. **Tres acotaciones que la decisión necesita — las dos primeras son números medidos, la tercera es qué instrumento falta:** (1) **1374 es un piso, no el número final** — sale de las fixtures del corpus, no de fichas reales; una descripción más larga o un cuarto punto de retiro lo suben, y `listing-view.ts` recorta la descripción pero **esa cota superior no está medida**; (2) **con 1374 el techo absoluto del chat queda en USD 0,8795 contra 1,00 — 1,14× de headroom**, así que esta fila se lee junto con `T50`: subir el techo es barato, subirlo *sin* haber clavado el de llamadas facturadas ya no tanto. (3) **`C6` antes que el techo, y ya tiene fila: `T53`.** No es orden de trabajo, es **qué información tiene el que decide**: el 1374 sale de cuatro fixtures y **hoy nada mide si una ficha real degrada**, porque el `ContextTrimReport` se construye y no lo lee nadie. **Decidir hoy es decidir sobre un corpus sintético** — rama legítima y con precio medido, pero conviene que sea elegida y no heredada. **Y el dato que cambia cómo se lee todo lo anterior, verificado en el archivo por `docs-keeper` y no citado de segunda mano: subir el techo NO es una variable de entorno, es un commit.** `packages/ai/src/env.ts:65-70` valida `LLM_MAX_INPUT_TOKENS` con un `.max()` contra la constante de `budget.ts`, y el mensaje de error dice literalmente *«la dieta se baja por env, nunca se sube»*. **Eso está bien y no es lo que hay que arreglar**: es la diferencia entre una perilla que alguien puede girar en producción y un número que se cambia con un diff revisado. Si la decisión es moverlo, lo que se toca es `budget.ts` | `packages/ai/src/budget.ts:12` (`MAX_INPUT_TOKENS = 1200`) · `packages/ai/src/env.ts:65-70` (`tokenCeiling`, el `.max()` y su mensaje) · `packages/ai/src/context.ts:153` (donde se aplica el límite) · `packages/ai/src/listing-view.ts` (el recorte de la descripción, cota superior **sin medir**) · `docs/COST.md` §2.8.4, §2.8.5 y `C9` · `docs/CHATBOT.md` (la dieta) · fila `T50` |
| T53 | **nadie cuenta cuándo la dieta se cae: el `ContextTrimReport` se construye y no lo lee ningún consumidor — y es lo que hace DECIDIBLE a `T52`** | todo · alias `C6` de `COST.md` · **es la fila de la que cuelga `T52`**, no al revés | `ai-agent` (`packages/ai/**`) la mitad que se puede hacer hoy · `app-agent` (`apps/web/app/api/**` y `(app)/**`) la de producción — los dos por §4 | la mitad de producción espera **FASE 5**: `/api/chat` no existe (mismo censo que `T51`). **La mitad del corpus no está bloqueada por nada.** Y ojo con el eco de `T51`, que ya hizo despachar trabajo inexistente una vez: **el campo YA está expuesto** —`index.ts` exporta `buildChatContext` y `type ContextTrimReport`, y `ChatAnswer.trimmed` viaja en la respuesta—, así que lo que falta no es exponer, es **contar** | **Origen: `cost-auditor`, `docs/COST.md` §2.8.5 punto 3 y la tabla de `C6`.** `buildChatContext` arma un `ContextTrimReport` en cada turno —qué se tiró para que el prompt entrara en el techo— y **no lo lee ningún consumidor**: censado contra HEAD `6aea02b`, cero llamadores de `.trimmed` fuera de los tests del propio paquete. **Por qué esta fila no es higiene ni prerequisito de proceso —lo corrigió el LEAD al despacharla, y la corrección es la fila—: `C6` es lo que hace DECIDIBLE a `T52`.** El **1374** sale de **cuatro fixtures**, no de fichas de tenants reales, y hoy **no existe forma de saber si una ficha real degrada**, porque el único instrumento que lo diría se construye y se tira. Traducido: **si el humano decide el techo hoy, lo decide sobre un corpus sintético; con `C6` lo decide sobre sus propios tenants.** **Y hay que decir qué compra cada mitad, porque no compran lo mismo:** **(a)** contar la degradación sobre el corpus vive adentro de `packages/ai` y se puede hacer **ya** — compra convertir *«9 de 162, 7 la ficha del plan Negocio»* de **medición de una tarde** en **afirmación que se pone roja sola**, y el board ya tiene el precedente escrito en `T43`: **medido no es testeado**; **(b)** la que contesta *«¿degradan las fichas de MIS tenants?»* es la emisión al log en producción, y **ésa espera FASE 5**, exactamente como `T51`. **Dos números que explican por qué el contador importa más que el número del techo, los dos medidos por `cost-auditor`:** entre §2.7 y §2.8 —**el mismo día**, sin que nadie tocara `budget.ts`— el peor prompt sin degradar pasó de **1251 a 1374**, o sea que **la variable se mueve sola**, empujada por fixtures y por features, y un techo elegido sin contador te deja en el mismo lugar la próxima vez que la ficha crezca 200 tokens. Y hoy degradan **9 de 162 prompts armados, 7 de ellos la ficha del plan Negocio**: el tenant de **USD 35 es el que más pierde**, que es lo que hace que esto sea calidad y no ruido. **Lo que esta fila NO decide:** qué se hace con la cuenta una vez que exista —umbral, alarma, o gate de la eval— no está decidido, y `docs-keeper` no lo cierra | `packages/ai/src/context.ts:92-105` (`ContextTrimReport` y dónde se arma) · `packages/ai/src/chat.ts:165` (`ChatAnswer.trimmed`, que ya viaja en la respuesta) · `packages/ai/src/index.ts` (**ya lo exporta: falta el consumidor, no el export**) · `apps/web/app/(app)/_lib/log.ts` (`logEvent`, el sink de la mitad (b)) · `docs/COST.md` `C6` y §2.8.5 punto 3 · filas `T52` (la decisión que esto vuelve decidible) y `T51` (misma forma, mismo bloqueo) |
| S8.2 | `listing.title` llega al prompt **sin** `sanitizeForPrompt` mientras `description` sí pasa | **doing** · **misma entrada que `T50`, no una segunda**: el LEAD la sumó el 2026-08-28 al encargo que `ai-agent` ya tenía abierto, así que `packages/ai` tiene **un** `doing` con dos ítems (ver §Estados) · encargo despachado por el LEAD el 2026-08-28 · **severidad baja** | `ai-agent` (`packages/ai/**` por §4) | — | **La cadena es de tres saltos y hay una decisión humana en el medio, que es lo que la baja a low:** el visitante escribe `model_text` en el formulario público → el dueño lee el lead en el inbox → el dueño **acepta** y publica la unidad → ese texto termina en `listings.title` → `title` entra al prompt del chatbot. O sea que no hay camino directo de un anónimo al prompt: hace falta que el dueño acepte. **Lo que igual la hace fila:** la asimetría ya está escrita en el código —`description` pasa por `sanitizeForPrompt` y `title` no—, así que hoy la protección depende de que ningún dueño publique sin leer, y S8 acaba de crear el primer camino por el que texto de un tercero llega a `title` sin que el dueño lo haya tipeado él. `ARCHITECTURE.md` §"Límites de confianza" dice *"dueño (texto libre) → prompt del LLM: **sanitizado y delimitado**"*, sin distinguir campos | `packages/ai/src/**` (donde se arma el prompt y vive `sanitizeForPrompt`) · `packages/domain/src/dto.ts` (`title` en la allowlist pública) · `apps/web/app/(app)/_lib/tradein/accept-to-stock.ts` (el salto que S8 agregó) |
...
### S2.5 · el guard de IMEI rechaza keys legítimas  ·  `media-agent`  ·  **ABIERTA, arreglo en vuelo**

> **Qué es:** un defecto de disponibilidad en `packages/media`, medido, que ataca directo el *done
> cobrable* de `CLAUDE.md` §1. **Para quién:** `media-agent`, que lo está arreglando mientras se
> escribe esto, y el LEAD, que va a re-ejecutar. **Cuándo se actualiza:** cuando el arreglo esté
> commiteado y corrido. **Esta sección no afirma que esté arreglado.**
...
**Lo que este board NO decide.** Cómo se arregla es de `media-agent`, y lo está haciendo. Lo que sí
queda escrito como criterio de cierre, porque es lo que este repo aprendió a pedir:
...
**La fila sigue abierta igual, y ahora por la mitad que falta de verdad:** `CLAUDE.md` §0 pide
artefacto commiteado **y** re-ejecución del comando de aceptación por el LEAD. La primera mitad se
cumplió; la segunda no. Que el motivo del "abierto" cambie sin que cambie el estado es exactamente
por qué este board escribe **qué falta** al lado del estado, y no sólo el estado.
...
#### El requisito de aceptación que nace ANTES que la slice — agregado el 2026-08-28 al cerrar S6.2
...
| hecho, verificado contra `main` el 2026-08-28 | consecuencia |
|---|---|
...
| `invalidateStorefront()` tiene **un solo caller en todo el repo**: `create-tenant.ts:378` | no hay ningún ejemplo vivo de "editar config del tenant" del que copiarse |
...
**Gate sugerido para la slice, que ya existe como argumento en §S5:** la aserción que le falta a S5 es
**causal** —perturbar el TC del tenant y ver moverse el ARS servido— y hoy no se puede escribir
porque mover el TC exige recrear el negocio. T12 es lo que la vuelve escribible; conviene que la
slice la traiga.
...
| fila | qué | dueño |
|---|---|---|
| **P5** | creció: `membership_role` tampoco aparece en las policies de `tradein_leads`, así que la regla 9 sobre esa tabla la sostiene el servidor | `db-agent` |
...
| **T39** | `accepts_trade_in` no tiene UI de edición: no se puede **dejar** de recibir canjes | `app-agent` |
| **T40** | `readBody()` bufferea antes del techo — mismo patrón que `/api/track`, sin declarar | `storefront-agent` |
...
| **T47** | `proxy.ts:363-365` explica una fragilidad del parser de matchers que ya no existe | `storefront-agent` |
| **T48** | el header de la `0009` se autotitula `S9`, y `S9` es otra slice | `db-agent` |
...
| **S8.2** | `listing.title` llega al prompt sin `sanitizeForPrompt` | `ai-agent` · `doing` **en la misma entrada que `T50`** |
| **T50** | el techo de llamadas facturadas por turno es **4** y no hay constante ni test que lo diga | `ai-agent` · `doing` (**con `S8.2`, una sola entrada**) |
| **T51** | nadie emite `billed` ni alarma en `calls > 2` — y la mitad que `C10` le pedía a `ai-agent` **ya estaba hecha** | `app-agent` (el log, con FASE 5) |
...
**S8.2** (`listing.title` llega al prompt sin `sanitizeForPrompt`), que sigue **abierta**. La otra era
...
Lo que ese test **no** cubre es S8.2: ahí el problema no es PII, es contenido sin sanitizar.

**Y cuatro filas más aterrizan acá, las cuatro de la auditoría de costo del 2026-08-28:** **`T50`**
(el techo de 4 llamadas facturadas por turno, sin constante ni test), **`T51`** (emitir `billed` al
log con alarma en `calls > 2`), **`T52`** (el techo de 1200 tokens de entrada, `blocked` por
...
> criterio lo da la columna *bloquea* de esta tabla: los seis blockers son **insumos externos que el
