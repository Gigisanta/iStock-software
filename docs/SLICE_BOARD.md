...
| S8.2 | `listing.title` llega al prompt **sin** `sanitizeForPrompt` mientras `description` sí pasa | **doing** · encargo despachado por el LEAD el 2026-08-28 · **severidad baja** | `ai-agent` (`packages/ai/**` por §4) | — | **La cadena es de tres saltos y hay una decisión humana en el medio, que es lo que la baja a low:** el visitante escribe `model_text` en el formulario público → el dueño lee el lead en el inbox → el dueño **acepta** y publica la unidad → ese texto termina en `listings.title` → `title` entra al prompt del chatbot. O sea que no hay camino directo de un anónimo al prompt: hace falta que el dueño acepte. **Lo que igual la hace fila:** la asimetría ya está escrita en el código —`description` pasa por `sanitizeForPrompt` y `title` no—, así que hoy la protección depende de que ningún dueño publique sin leer, y S8 acaba de crear el primer camino por el que texto de un tercero llega a `title` sin que el dueño lo haya tipeado él. `ARCHITECTURE.md` §"Límites de confianza" dice *"dueño (texto libre) → prompt del LLM: **sanitizado y delimitado**"*, sin distinguir campos | `packages/ai/src/**` (donde se arma el prompt y vive `sanitizeForPrompt`) · `packages/domain/src/dto.ts` (`title` en la allowlist pública) · `apps/web/app/(app)/_lib/tradein/accept-to-stock.ts` (el salto que S8 agregó) |
...
| **S8.2** | `listing.title` llega al prompt sin `sanitizeForPrompt` | `ai-agent` · `doing` |
