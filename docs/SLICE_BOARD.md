...
| T17 | la reserva **configurable por tenant** está prometida y no existe | todo | `app-agent` | — | `PRODUCT.md` P3 promete *"duración default de la reserva, configurable"*; hoy el rango y el default son **constantes del dominio** (`RESERVATION_MIN/MAX/DEFAULT_MINUTES`) más el `CHECK` `reservations_minutes_range`, iguales para todos los tenants. Cerrada cuando el dueño fija la duración default de **su** negocio desde el panel, dentro de \[30, 120\], y fuera de rango se sigue **rechazando y no clampeando**. **Sin fecha** — no bloquea ninguna slice de FASE 4 | `packages/domain/src/reservation.ts` · `apps/web/app/(app)/app/(panel)/ajustes/` |
...
>
> **Y la colisión ya no es hipotética: está en `main`.** `5f9ca03` —el commit del arnés de `guard-effects.sh`, posterior a los seis de S7— se titula **`[fix] T32: …`**, o sea que el repo tiene hoy **dos `T32`**: la fila `done` de `guard-doc-tables.sh` (`d3deb86`) y ese commit. Igual con `T31`, que este board usa para el evento de cuarentena del cron y el encargo usaba para las probes. **`docs-keeper` no reescribe mensajes de commit ni renumera filas `done`**; lo deja medido y a la vista para que el LEAD elija: o los ids nuevos son `T33`/`T34` como quedaron acá, o se renumera lo viejo y hay que tocar `INDEX.md`, el título de §T21–T25 y las celdas que citan `T31`.
...
| aplicación | las 2 reglas corriendo y **disparando** un `429` | 🔴 **no aplicadas**. No hay proyecto Vercel (**B2**/**B5**). Desde S4 (`c9611b1`) `storefront-track-rl` está `active` —su ruta ya existe— y `chatbot-rl` sigue `planned` porque `/api/chat` aterriza con **FASE 5** |
