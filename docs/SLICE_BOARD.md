...
| S13 | `/demo` | **esperando gate** · 2026-08-28 · entregado por `storefront-agent` y **no verificado contra un server corriendo**: el `308` se probó sobre el objeto `NextResponse` en unit (`app/(storefront)/demo.test.ts`), no sobre la red — lo declaró el propio agente y se registra así en vez de redondearlo. **No la cierra quien la escribió** (**ADR-022** aplicado al board) | `storefront-agent` | tenant demo aislado; cero datos reales. **Diseño ratificado por el LEAD el 2026-08-28 → ADR-027**: `maat.work/demo` y `/demo/**` emiten un **308 permanente** a `demo.<apex>/…` desde `proxy.ts` (`demoAliasRedirect`), **adentro de la rama `marketing` de `resolveHost`** y con el host de destino derivado del **entrante** (`tenantHostFor`, en `app/(storefront)/_lib/host.ts`); los tres motivos por los que se descartó el rewrite los preserva el ADR. **Lo que falta, y lo corre el LEAD — no `storefront-agent`:** **(1)** `scripts/accept-s13.sh`, que **todavía no existe** y es del LEAD por §4; **(2)** `bash scripts/guard-routes.sh`, que lee `.next/prerender-manifest.json` y por lo tanto **requiere `next build`**; **(3)** un e2e que mida el `308` y el `Location` **sobre la red**, hoy inexistente. **`B6` sigue abierto y muerde por otro lado**: sin `SEED_DEMO_WA_PHONE` el seed cae en `SEED_DEMO_WA_PHONE_FALLBACK`, así que el `wa.me` del demo lleva un teléfono placeholder — no bloquea el `308`, sí bloquea *«demo mostrable a un prospecto»* |
...
| T56 | **`DEMO_TENANT_SLUG` es un segundo literal `'demo'` viviendo en `apps/web`: su casa canónica es `packages/domain`, al lado del Set que lo protege** | todo · **pedido de `storefront-agent` a otra columna, salido de S13** — esa columna *pide, no edita* (§4) · severidad baja **porque la copia no está sostenida por una promesa de review**: hoy la atan tres aserciones ejecutables, así que si el Set cambia el rojo llega en el commit | `domain-agent` (`packages/domain/**` por §4) | — | la constante se exporta desde `packages/domain/src/reserved-slugs.ts` y `apps/web` la **importa** en vez de declararla; las tres aserciones que hoy la atan siguen verdes o se mudan con ella | `apps/web/app/(storefront)/_lib/host.ts` (`DEMO_TENANT_SLUG`, con el motivo ya escrito arriba de la constante) · `apps/web/app/(storefront)/demo.test.ts` (bloque *«`DEMO_TENANT_SLUG` es una copia ATADA de lo que decide `packages/domain`»*, 3 tests) · `packages/domain/src/reserved-slugs.ts` (`TENANT_SERVED_RESERVED_SLUGS`, que ya contiene `'demo'`) |
...
| B6 | número de WhatsApp del tenant demo — **ya tiene forma de env**: entra por `SEED_DEMO_WA_PHONE` y sin él `packages/db/src/seed.ts` cae en `SEED_DEMO_WA_PHONE_FALLBACK`, que es un placeholder | S4, S13 | **humano** |
