...
> (`s8-canje.test.ts:59`), o sea que el comando de aceptación **muta la base de desarrollo de quien
...
`guard-routes.sh` más **siete aceptaciones por slice**: `accept-s1.sh` (`:302`), `accept-s2.sh`
(`:306`), `accept-s3.sh` (`:310`), `accept-s4.sh` (`:314`), `accept-s6.sh` (`:323`),
**`accept-s7.sh` (`:336`)** y **`accept-s8.sh` (`:346`)**. **No hay `accept-s5.sh`** — es deuda de
proceso declarada, no un olvido: `SLICE_BOARD.md` §S5.

**Los números de línea de este párrafo se releyeron el 2026-08-28 después de cerrar S8**
(`grep -n 'accept-s\|lint.test.sh' .github/workflows/ci.yml`), y **cambiaron todos**: los que decía
antes eran de `10d31b6` y quedaron corridos ~78 líneas. Se dejan porque ubican, no porque
identifiquen; **el que identifica es el nombre del script**, y el que censa que esté en `ci.yml` es
`guard-gates.sh` §**G4**, no esta lista.

**Y desde S8 el quinto lint tiene arnés: `scripts/rls-lint.test.sh` (`:192`), 12 casos**, que cierra
