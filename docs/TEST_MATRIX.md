...
**Última verificación CI documentada:** CI run `33457409060` (concluido el 2026-09-01 UTC), sobre
el commit anterior al hardening de esta corrida. Los conteos de esa corrida son históricos; no
representan cobertura de producción ni de integraciones externas reales.
...
**Conteo archivado — 2026-08-28, después de S8.1, medido por el LEAD sobre el árbol:**
...
corridos por `qa-agent`; esa era la medición de esa fecha.

**Conteo local ejecutado — 2026-09-01 UTC:** `packages/domain` **244** en **13** archivos ·
`packages/media` **164** en **11** · `packages/ai` **591** en **20** · `packages/db` **473** en
**17** · `tests/` **432** en **9** · `apps/web` **985** en **61**, más **4 skipped**. El gate
local reportó **2889 tests corridos**; los cuatro skips pertenecen a los experimentos de ADR-008.
...
> **Registro histórico de la medición previa a CI:** el `79` de arriba es una cifra de corrida
> fechada, y S7 la movió: no se re-suma acá. R9 son
...
> En ese corte, **`pnpm e2e` todavía no se había corrido** (requería `next build`) y no se contaba
> como verde. El estado actual está documentado en la sección e2e y en el CI run `33457409060`.
>
...
**Frontera de lecturas sensibles — estado de HEAD `bfc6228`:** `packages/db/src/seller-authorization.test.ts`
verifica contra Postgres que el `SELECT` directo de `listings.cost_usd` y de
`tradein_leads.offer_usd`/`internal_notes` está revocado para `authenticated`, y que el owner sólo
recibe esos datos por `owner_get_listing_cost` y `owner_get_tradein_sensitive`; el seller recibe
cero filas por esos RPC. Las queries server-side del panel además prueban que el objeto seller no
lleva costo ni los campos sensibles. En el CI run `33457409060`, `packages/db` reportó **462 tests
en 15 archivos** y `apps/web` **973 tests en 59 archivos + 4 skipped**. Esto cubre Postgres local y
objetos/queries server-side; no cubre un payload de red del panel ni una integración de producción.
...
## e2e — Playwright
Estado verificado contra `e2e/**` y `scripts/accept-*.sh` en el CI run **`33457409060`**, sobre HEAD
`bfc6228` (2026-09-01 UTC). La suite corrió **94 tests · 0 skip**, en **15 archivos**, y el censo
del reporter dijo **15/15 archivos** y **94/94 tests ejecutados**. El censo lo emite el reporter de
`qa-agent` y lo lee M4 de `accept-s2.sh`; **no se cuenta por nombre de archivo en la salida**, porque
Playwright imprime el nombre de un spec que no corrió.
...
>    nada.
> 3. La corrida de CI usa un Postgres descartable y `next start` local; en el job e2e además fija
>    `MEDIA_DRIVER=local` y `BILLING_DRIVER=mock`. No prueba R2, Mercado Pago, LLM ni otra
>    integración externa real, y tampoco permite afirmar cobertura de producción.
...
| E2 | **otro browser** (sin sesión) abre `{slug}` y entra a una ficha | los 15 campos presentes | 🟡 **no por browser** — los campos los mide **`curl`** en M3/M3b/M4 de `accept-s3.sh`, sobre los bytes servidos bajo el host del tenant. Es una cobertura fuerte (lee el payload de RSC, donde un objeto crudo se escapa sin verse) pero **no prueba lo que un browser hace con ellos**: ni JS, ni layout, ni el click. La corrida `33457409060` sí ejecutó `accept-s3.sh` y cerró S3, pero el gate sigue siendo `curl`, no browser |
| E3 | click en WhatsApp | URL con el **texto exacto** del producto y el precio | ✅ **cerrada por S4**: `e2e/s4-el-click-de-whatsapp-deja-una-fila-sin-pii.spec.ts` carga la ficha con JavaScript apagado y compara el `href` completo contra el teléfono, producto, condición, precio y host esperados. También afirma exactamente un `wa.me`, `target="_blank"` y `rel="noopener"`; la prueba de click sigue verificando que el beacon no bloquee la navegación ni duplique el evento |
| E4 | unidad `reserved` | badge visible; **no** dice "disponible"; copy alternativo | ✅ **cerrada por S4 + S6**: S6 afirma el badge y la invalidación de la ficha cacheada; el segundo caso browser de `e2e/s4-el-click-de-whatsapp-deja-una-fila-sin-pii.spec.ts` afirma sobre una ficha reservada real `Reservado`, cero `Disponible`, un único botón y el `href` completo del copy de segunda intención |
| E5 | canje: form público → inbox → checklist → aceptar | unidad creada en `draft` con costo | ✅ **cerrada por S8**: `e2e/s8-el-canje-llega-al-inbox-y-entra-como-borrador.spec.ts` usa contextos de browser separados para visitante y dueño, envía el formulario público sin JavaScript, recorre el inbox, selecciona catálogo y variantes, acepta y comprueba en Postgres que la unidad queda en `draft`, con el tenant y título correctos |
| E6 | login como **seller** | `cost_usd` **ausente del payload de red**, no sólo de la pantalla | ✅ **cerrada el 2026-09-04** — `e2e/s11-seller-payload.spec.ts` hace login seller por navegador, resuelve una membresía real, muestra un equipo cuyo costo e IMEI existen en la fixture y lee el body HTTP de `/app/stock`: `costUsdCents`, `cost_usd`, `marginUsd`, `margin_usd`, `internalNotes`, `internal_notes` y el IMEI están ausentes. La misma corrida entra a `/billing` y ve `data-auth="forbidden"` con la pantalla de permiso. `E2E_ALLOW_PARTIAL=1 E2E_PORT=3129 pnpm --filter @istock/e2e e2e s11-seller-payload.spec.ts` → **1 passed**. Con Cache Components esa página puede conservar status 200 porque transmite el shell antes del boundary; el POST protegido de suscripción conserva 403 en `billing/subscribe/route.test.ts`. |
...
| E10 | peso de la imagen `card` en la grilla | **< 200KB** medido en la respuesta de red | ✅ **medido en CI run `33457409060` (2026-09-01 UTC)**: `transferSize=51016B` contra un techo de 204800 B, viewport 390×844 dpr 3, variante `card`. `s3-la-grilla-…` + M2 de `accept-s3.sh` |
| E11 | LCP mobile de la ficha (4G simulado) | dentro del presupuesto de `ARCHITECTURE.md` | 🔴 **sin cubrir**: T13 ya está cerrado para el driver local (`/_media` emite `Timing-Allow-Origin: *` y el e2e de media lo verifica), pero todavía no hay una traza LCP con throttling ni una medición del CDN productivo. El MCP de Chrome DevTools no está configurado en este entorno y el header de `img.maat.work` queda UNVERIFIED hasta el deploy |
| E12 | mutar precio en el panel → recargar vidriera | precio nuevo **sin esperar TTL** | ✅ **cerrada el 2026-09-04** — `updateListingPrice()` toma el lock de la unidad, filtra por tenant, escribe el precio y el evento `price_change`, y `updateListingPriceAction()` pasa el slug confiable de la sesión a `invalidateStorefrontUnit()`; la prueba E2E de `s12-onboarding-primer-equipo-publicado.spec.ts` cambia USD 620 a USD 625 y comprueba el valor nuevo en la grilla pública inmediatamente. La suite completa quedó en **109/109 tests, 19/19 specs, sin skips** |
...
| E18 | visitante sin sesión abre `/billing/suscribirse?plan=base` | redirige a `/ingresar?plan=base` y muestra el plan elegido | ✅ **cerrada el 2026-09-04** — `e2e/s0-marketing-landing.spec.ts` navega por el endpoint público y afirma status 200, destino de login y copy de Base; cubre el contrato que el preflight inspecciona en el deployment |
| E19 | dueño edita configuración visible → recarga la vidriera | el nombre actualizado llega al host público sin recrear el tenant | ✅ **cerrada el 2026-09-04** — `e2e/s12-onboarding-primer-equipo-publicado.spec.ts` guarda desde `/app/ajustes`, espera `Guardado.` y abre el subdominio de la misma fixture; prueba la invalidación después del commit |
...
**`accept-s8.sh`**, **`accept-s9.sh`**, **`accept-s13.sh`** y **`accept-s10.sh`**: diez aceptaciones
por slice. **No hay `accept-s5.sh`** — es deuda de proceso declarada, no un olvido: `SLICE_BOARD.md`
§S5.
...
> ### Historial: `ci.yml` todavía no había corrido al 2026-08-28
>
> ```
...
> ```
>
> `origin` estaba configurado sin ramas y `origin/main` figuraba `gone`. En **110** commits locales
> todavía no había una corrida de GitHub Actions; se re-midió sobre `d3deb86` el 2026-08-28. Los
> cuatro steps nuevos de `guard-gates` y `guard-doc-tables` nacieron en esa condición. Ese era el
> estado histórico; no describe el HEAD actual.
>
> | se lee | significa |
> |---|---|
> | ✅ / *"en cada push"* | **nivel 1**: el gate pasa a mano y `ci.yml` declara su step |
> | ✅ con run identificado | **nivel 2**: corrió en `ubuntu-latest` sobre ese commit |
> | — | no hay evidencia de una corrida del gate |
>
> La distinción sigue siendo la de ADR-016: el archivo declara, no ejecuta. En el HEAD actual,
> `origin/main` apunta a `bfc6228` y el CI run **`33457409060`** terminó **success**: el job de
> build (`typecheck · lint · test · rls`) y el job `e2e` terminaron **success**. El primero reportó
> FASE 2 y FASE 3 en PASS; el segundo reportó Playwright **94/94 en 15/15** y ejecutó las
> aceptaciones S1, S2, S3, S4, S6, S7, S8, S9, S13 y S10 en verde.
>
> Esto prueba la corrida de CI sobre este commit, no producción: usa Postgres descartable, un
> `next start` local y, en e2e, `MEDIA_DRIVER=local` y `BILLING_DRIVER=mock`. No prueba R2,
> Mercado Pago, LLM ni integraciones externas reales. El nivel 2 no se extiende a esos sistemas.
...
| prohibición de §2 | quién la afirma hoy | ¿tiene step en CI? (**nivel 1**; nivel 2 si hay run identificado) |
|---|---|---|
| `tenant_id` en `user_metadata` | **estático:** `guard-leaks.sh` · regla 7 (`user_metadata`) · `web-lint.mjs` (W008) · `accept-fase3.sh` · *“tenant_id JAMAS en user_metadata”* — **y en runtime:** `tests/rls-cross-tenant.test.ts` · *“un claim con el tenant en `user_metadata` … no abre nada”*, que **forja un claim** con el tenant en `user_metadata` contra Postgres real y verifica que **no abre nada** | ✅ nivel 2 en `33457409060` |
| tabla nueva sin `GRANT` | `guard-grants.sh` (parsea por **sentencia**, no por línea: 5 de los 6 `GRANT` son multilínea) — **y en runtime:** R7a/R7b/R7c preguntan por el privilegio **efectivo** (`has_table_privilege`), así que también cae un `GRANT … TO PUBLIC` | ✅ nivel 2 en `33457409060` |
| **`GRANT` de `INSERT` por columna incompleto** — la tabla **tiene** `GRANT` y el panel igual recibe `42501` | **`G6`** · `scripts/probes/el-grant-cubre-el-insert-de-drizzle.test.ts`, sección **D5** de `accept-fase2.sh`. Le pregunta al catálogo (`has_column_privilege`) tabla de negocio por tabla de negocio; **cero privilegios = fuera de alcance**, esa tabla la escribe `service_role`. Trae control de polaridad propio, siempre encendido | ✅ nivel 2 en `33457409060` (`accept-fase2.sh` con Postgres migrado) |
...
>
> **Actualización, CI run `33457409060` sobre HEAD `bfc6228` (2026-09-01 UTC):** el estado
> *"sin commitear · cero commits"* y la medición de `origin` vacío eran históricos al 2026-08-28.
> El run actual terminó **success** y aporta evidencia nivel 2 para sus jobs y steps; no convierte
> esa evidencia local en cobertura de producción ni de integraciones externas reales.
