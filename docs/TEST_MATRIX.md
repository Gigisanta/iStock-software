...
| E3 | click en WhatsApp | URL con el **texto exacto** del producto y el precio | 🟡 **ahora sí hay un browser, y el texto sigue sin estar afirmado entero sobre el camino real** — cambió con S4 (`c9611b1`): `e2e/s4-…-sin-pii.spec.ts` · *“con JavaScript apagado la ficha servida trae el único enlace a WhatsApp”* lee la ficha con `javaScriptEnabled: false` y mide `anchors=1 · abre_whatsapp=si`, y *“mirar la ficha no registra nada y recién el click…”* **hace el click**. Lo que ninguna de las dos hace es **comparar el `href` completo**: W5 lo **imprime**. Sumado a M3b de `accept-s3.sh` (substrings sobre el HTML servido: un solo anchor en la ficha, cero en la grilla, teléfono contra `SEED_DEMO_WA_PHONE`, `USD 620` + `demo.maat.work` + `y lo quiero.`, `usado A` sí / `usado excelente` no) y a U14 en unit (`toBe`, pero con el `modelDisplayName` ya limpio), quedan **tres pruebas alrededor del string y ninguna encima** — que es exactamente cómo pasó **S4.1**. `accept-s3.sh` corrió en el job e2e del CI run `33457409060`; la ausencia es la aserción del `href` completo |
...
| E5 | canje: form público → inbox → checklist → aceptar | unidad creada en `draft` con costo | 🔴 **sin cubrir por browser** — la corrida `33457409060` ejecutó Playwright **94/94 en 15/15 specs** sin un escenario de canje; en el mismo job `accept-s8.sh` cerró `S8: ACEPTADA` y midió contra Postgres local. `scripts/probes/s8-canje.test.ts` sigue afirmando las tres puntas (`lead_anonimo_entra=1`, `accept_crea_unidad_en_draft=1`, `accept_dos_veces_una_unidad=1`), pero eso no reemplaza una prueba browser del recorrido. |
| E6 | login como **seller** | `cost_usd` **ausente del payload de red**, no sólo de la pantalla | 🔴 **sin cubrir por browser** — S11 ya dejó pruebas server-side en `apps/web/app/(app)/_lib/listings/queries.test.ts`: el objeto seller no lleva `costUsd`, margen ni sensibles, y el owner conserva costo; `apps/web/app/(app)/_lib/tradein/queries.test.ts` cubre oferta/notas en el objeto y en el SQL de la rama seller. Sigue sin existir un spec de e2e con login seller ni una aserción sobre el payload HTTP; los `costUsd` sembrados en e2e no prueban ausencia. |
...
| **una key legítima rechazada por el guard de PII** (disponibilidad, no seguridad) | hoy `packages/media/src/keys.test.ts` — **y hasta el 2026-08-28 no lo cubría nadie**: los casos del gate eran keys elegidas a mano, todas sin 15 dígitos seguidos, así que el 0,63% de keys que el guard rechaza **nunca apareció en un test**. Es un hueco de *muestreo*, no de olvido: la forma que faltaba era **generar keys reales en masa** y afirmar sobre el efecto | 🔴 **S2.5, abierta según `SLICE_BOARD.md`** — el arreglo está en `main` (`1fc0e59`, `6e74a51`) y el CI run `33457409060` ejecutó `packages/media`: **164/164 tests en 11 archivos**, incluido `keys.test.ts`. También pasó `accept-s2.sh`; no existe en CI un gate separado cuyo veredicto el board reconozca como aceptación específica de S2.5, así que este documento no la eleva a ✅. **La lección que sí es de este doc:** un guard cuyos casos de prueba los escribe la misma persona que el regex sólo ve las entradas que esa persona imaginó; el defecto vivía en las que produce la máquina |
