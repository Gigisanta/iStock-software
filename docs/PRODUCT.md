...
| Q3 | ¿Duración default de la reserva? | 60 min (rango permitido 30–120). **Implementado en S6, a medias:** el rango y el default son constantes del dominio (`RESERVATION_MIN/MAX/DEFAULT_MINUTES`), sostenidas además por el `CHECK` `reservations_minutes_range`; **el "configurable por tenant" no existe**. Fuera de rango se **rechaza**, no se clampea. **Desde el 2026-08-28 la mitad que falta tiene fila: `T17` en `SLICE_BOARD.md`.** |
