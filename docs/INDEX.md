...
| [DECISIONS.md](DECISIONS.md) | ADRs numeradas con alternativas descartadas y verificación | LEAD en FASE 1, después `docs-keeper`; **el LEAD ratifica** cada ADR nueva | **ADR-001..014; 008 y 010 abiertas.** ADR-011 supersede el corolario 4 de ADR-007; ADR-012 lo precisa con el polo negativo; **013** (indistinguibilidad en el panel) y **014** (`instant = false`) salieron de S2 |
...
pasaron a `done`** con la corrida registrada en `SLICE_BOARD.md`. **FASE 3 (K1–K5), S1 y S2 NO**: sus
gates terminan en `next build` y el `next build` está roto (`usePathname()` fuera de `<Suspense>` en
`/app/stock/[id]/fotos`). El código está escrito, pero **el gate es la corrida, no el código**.
...
- **Antes de arrancar S3 hay tres cosas sin escribir**: `robots.txt`/`sitemap.xml` por tenant
  (**P1**), los file conventions de metadata bajo host de tenant (**P2**), y el gate que mide **el
  byte que el browser pide** — un `srcset` sin `sizes` baja `detail` (128.570 B) donde el
  presupuesto dice `card` (50.692 B), y el criterio «`card` ≤150 KB» quedaría midiendo el byte que
  nadie descarga (**P3**).
- **Deuda de S2:** `collectOrphanObjects` existe, está testeado y **no lo llama nadie**, mientras
  ...
  y no por efecto (**T5**, **T6**); el `<input type="file">` no se limpia tras subir (**S2.3**).
...
- **Deuda de los instrumentos, abierta en FASE 4:** el test de RLS cruzado vive en el directorio de
  `db-agent` y lo tiene que auditar `qa-agent` → se muda a `tests/` después de S2 (**T3**); y
  `none()` está copiado en tres gates con dos versiones distintas → `scripts/_lib.sh` (**T4**).
