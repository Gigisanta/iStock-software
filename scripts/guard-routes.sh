...
const ESPERADO = {
  ...
  // Identity routes generated by the App Router. They are public, immutable brand assets and
  // intentionally static; keep them in this table so adding a favicon cannot look like an
  // unreviewed route drift.
  "/apple-icon.png":        "static/complete",
  "/icon.svg":              "static/complete",
  "/manifest.webmanifest": "static/complete",
  "/opengraph-image.png":   "static/complete",
  "/twitter-image.png":     "static/complete",
...
for (const [ruta, modo] of real) {
  // Use the segment boundary: `/apple-icon.png` is a public identity asset, not `/app/*`.
  if (ruta !== "/app" && !ruta.startsWith("/app/")) continue;
